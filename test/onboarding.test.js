...
check('changing a recognized ticker to an unknown ticker clears the automatic class',()=>{const classes=new Set();const elements={'f-ticker':{value:'VOO'},'f-cls':{value:'',disabled:true,setAttribute(){},removeAttribute(){}},'holding-class-field':{classList:{add:c=>classes.add(c),remove:c=>classes.delete(c),toggle:(c,on)=>on?classes.add(c):classes.delete(c)}},'f-cls-help':{textContent:''}};sandbox.document.getElementById=id=>elements[id]||null;T.syncHoldingClassField();assert.equal(elements['f-cls'].value,'us_large');assert.equal(elements['f-cls'].disabled,true);elements['f-ticker'].value='ZZUNKNOWN';T.syncHoldingClassField();assert.equal(elements['f-cls'].value,'');assert.equal(elements['f-cls'].disabled,false);assert.equal(classes.has('is-required'),true);sandbox.document.getElementById=()=>null;});
